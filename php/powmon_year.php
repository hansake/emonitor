...
echo "<head>";
echo "<title>Energy during $YEAR</title>";
echo "</head>";
echo "<body>";
echo "<H2>Energy consumption during $YEAR</H2>";
echo "<li>";
...
echo "</li>";
    ...
    $mdifftime = array_fill(0, $monthcntrs, 0);
    $mdiffcnt1 = array_fill(0, $monthcntrs, 0);
    $mdiffcnt2 = array_fill(0, $monthcntrs, 0);
    $mdiffcnt3 = array_fill(0, $monthcntrs, 0);
    $totcntr1 = 0.0;
    $totcntr2 = 0.0;
    $totcntr3 = 0.0;
    $totcntr4 = 0.0;
    ...
    $startdate = date_create("first day of January $YEAR");
    ...
    for ($i = $monthcntrs; 0 <= $i; $i--)
    {
        ...
        {
            $wmdiff1 = $wmdifflast1 - $row[1];
            $wmdifflast1 = $row[1];
            $wmdiff2 = $wmdifflast2 - $row[2];
            $wmdifflast2 = $row[2];
            $wmdiff3 = $wmdifflast3 - $row[3];
            $wmdifflast3 = $row[3];
            if ($i < $monthcntrs)
            {
                $mdifftime[$i] = $lasttimestamp;
                if ($wmdiff1 < 0)
                    $wmdiff1 = 0;
                $mdiffcnt1[$i] = $wmdiff1;
                $totcntr1 += ($wmdiff1 / 1000);
                if ($wmdiff2 < 0)
                    $wmdiff2 = 0;
                $mdiffcnt2[$i] = $wmdiff2;
                $totcntr2 += ($wmdiff2 / 1000);
                if ($wmdiff3 < 0)
                    $wmdiff3 = 0;
                $mdiffcnt3[$i] = $wmdiff3;
                $totcntr3 += ($wmdiff3 / 1000);
    ...
    $maxwatts1 = max($mdiffcnt1);
    $maxwatts2 = max($mdiffcnt2);
    $maxwatts3 = max($mdiffcnt3);
    $maxwatts = max($maxwatts1, $maxwatts2, $maxwatts3);

    /*Set parameters for width & height of graph and chart*/
    $wattspacing = 200000;
    $graphwidth = $monthcntrs * 70;
    $graphheight = 500;
    $topmargin =  30;
    $botmargin = 40;
    $leftmargin = 20;
    ...
    $xticker = floor($graphwidth / count($mdiffcnt1));
    ...
    for ($j = 0; $j < count($mdiffcnt1); $j++)
    {
        ...
        $ywatts1 = $botgraph - $mdiffcnt1[$j]*$scalefactorwatts;
        $polystringwatts1 = $polystringwatts1." ".$gx.",".$ywatts1;

        $ywatts2 = $botgraph - $mdiffcnt2[$j]*$scalefactorwatts;
        $polystringwatts2 = $polystringwatts2." ".$gx.",".$ywatts2;

        $ywatts3 = $botgraph - $mdiffcnt3[$j]*$scalefactorwatts;
        $polystringwatts3 = $polystringwatts3." ".$gx.",".$ywatts3;

        $ywatts4 = $botgraph - (($mdiffcnt1[$j] - ($mdiffcnt2[$j] + $mdiffcnt3[$j]))*$scalefactorwatts);
